...
var Cal={
    ...
    // Redirects to the standalone PHP order form on the Orders page.
...
// Simply redirects to the standalone PHP order form.
window.HM_openOrderPage=function(opts){
    var url = (HM.orders_url || '/orders/') + '?hm_action=create&patient_id=' + (opts.patient_id||0);
    if (opts.appointment_id) url += '&from_appointment=' + opts.appointment_id;
    window.location.href = url;
};
