...
var Cal={
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  EXCHANGE PAGE — Launched via ?exchange_id=X URL param
    // ═══════════════════════════════════════════════════════════════════
    ...
    _openExchangePage:function(exch){
        ...
        var patientId=exch.patient_id;
        var patientName=exch.patient_name||'Patient';
        var originalQty=exch.original_qty||1;
        ...
        post('get_order_products',{}).then(function(pR){
            ...
            (function buildExchange(){
                ...
                // ── Top bar — Exchange ──
                h+='<div style="background:#f59e0b;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 24px;height:50px;flex-shrink:0">';
                h+='<button id="hm-ex-back" style="background:none;border:1px solid rgba(255,255,255,.2);color:#fff;font-size:13px;font-weight:600;cursor:pointer;padding:6px 14px;border-radius:6px;font-family:var(--hm-font-btn);display:flex;align-items:center;gap:6px;transition:all .15s">';
                h+='<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 8H1M8 15L1 8l7-7"/></svg> Back to Patient</button>';
                h+='<div style="text-align:center"><div style="font-family:var(--hm-font-title,\'Cormorant Garamond\',serif);font-size:20px;font-weight:700;letter-spacing:-.3px">Exchange</div>';
                h+='<div style="font-size:11px;opacity:.8;margin-top:1px">'+esc(patientName)+' — '+esc(exch.exchange_number||'')+'</div></div>';
                h+='<div style="min-width:90px;text-align:right"><span style="font-size:11px;opacity:.6">'+esc(exch.reason||'')+'</span></div></div>';
                ...
                // Exchange info card
                h+='<div style="background:#fffbeb;border:2px solid #fde68a;border-radius:10px;padding:16px;margin-bottom:20px">';
                h+='<div style="font-size:12px;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;display:flex;align-items:center;gap:8px">';
                h+='<svg width="16" height="16" fill="none" stroke="#92400e" stroke-width="2"><path d="M7.5 21L3 16.5 7.5 12M16.5 21V16.5a3 3 0 00-3-3H3M16.5 3L21 7.5 16.5 12M7.5 3v4.5a3 3 0 003 3H21"/></svg>';
                h+=' Exchange Details</div>';
                h+='<div style="font-size:13px;color:#78350f;margin-bottom:4px"><strong>Device:</strong> '+esc(exch.device_name||'')+'</div>';
                if(exch.manufacturer)h+='<div style="font-size:12px;color:#92400e;margin-bottom:3px"><strong>Manufacturer:</strong> '+esc(exch.manufacturer)+'</div>';
                if(exch.serial_left)h+='<div style="font-size:12px;color:#92400e;margin-bottom:3px"><strong>Serial Left:</strong> '+esc(exch.serial_left)+'</div>';
                if(exch.serial_right)h+='<div style="font-size:12px;color:#92400e;margin-bottom:3px"><strong>Serial Right:</strong> '+esc(exch.serial_right)+'</div>';
                h+='<div style="font-size:12px;color:#92400e;margin-bottom:3px"><strong>Reason:</strong> '+esc(exch.reason||'')+'</div>';
                if(exch.notes)h+='<div style="font-size:12px;color:#92400e"><strong>Notes:</strong> '+esc(exch.notes)+'</div>';
                h+='<div style="margin-top:8px;padding-top:8px;border-top:1px solid #fde68a;display:flex;justify-content:space-between;align-items:baseline">';
                h+='<span style="font-size:11px;color:#78350f;font-weight:600">Original Value</span>';
                h+='<span style="font-size:16px;font-weight:700;color:#78350f">€'+creditAmount.toFixed(2)+'</span></div>';
                if(exch.prsi_amount>0)h+='<div style="display:flex;justify-content:space-between;font-size:11px;color:#92400e;margin-top:3px"><span>PRSI included</span><span>€'+parseFloat(exch.prsi_amount).toFixed(2)+'</span></div>';
                h+='<div style="margin-top:6px;font-size:11px;color:#92400e;font-style:italic">Qty: '+originalQty+' aid'+(originalQty>1?'s':'')+' — select '+originalQty+' new</div>';
                h+='</div>';

                // Product selection
                h+='<div><div style="font-size:12px;font-weight:700;color:var(--hm-text-light,#64748b);text-transform:uppercase;letter-spacing:.5px;margin-bottom:14px;display:flex;align-items:center;gap:8px">';
                h+='<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6"/></svg> Select New Device</div>';
                h+='<input type="hidden" id="hm-op-pid" value="'+patientId+'">';
                h+='<input type="hidden" id="hm-op-aid" value="0">';
                h+='<input type="hidden" id="hm-ex-id" value="'+exchId+'">';
                ...
                h+='<div id="hm-op-ear-wrap" style="display:none;'+LS+'"><label style="'+LB+'">Ear</label>';
                h+='<select id="hm-op-ear" style="'+INP+'">';
                h+='<option value="">— Select —</option><option value="Left">Left</option><option value="Right">Right</option>';
                if(originalQty>=2)h+='<option value="Binaural">Binaural (both)</option>';
                h+='</select></div>';
                ...
                // Add to order button
                h+='<div id="hm-op-add-wrap" style="display:none;'+LS+';text-align:right">';
                h+='<button type="button" id="hm-op-add-item" style="font-size:13px;font-weight:600;padding:8px 20px;border-radius:8px;border:none;background:var(--hm-teal,#0BB4C4);color:#fff;cursor:pointer;font-family:var(--hm-font-btn);transition:all .15s">+ Add to Exchange</button></div>';
                ...
                h+='<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px">';
                h+='<div>';
                h+='<div style="font-size:22px;font-weight:700;font-family:var(--hm-font-title,Cormorant Garamond,serif);color:#f59e0b;letter-spacing:1px">EXCHANGE</div>';
                h+='<div style="font-size:11px;color:var(--hm-text-muted,#94a3b8);margin-top:2px">'+esc(exch.exchange_number||'')+' &bull; '+(new Date().toLocaleDateString('en-IE',{day:'numeric',month:'short',year:'numeric'}))+'</div>';
                h+='</div>';
                h+='<div style="text-align:right">';
                h+='<div style="font-size:12px;font-weight:600;color:var(--hm-navy,#151B33)">'+esc(patientName)+'</div>';
                h+='<div style="font-size:11px;color:var(--hm-text-light,#64748b)">'+esc(exch.patient_number||'')+'</div>';
                h+='</div></div>';

                // ── OLD DEVICE — crossed out negative row ──
                h+='<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px 14px;margin-bottom:16px">';
                h+='<div style="font-size:10px;font-weight:700;color:#991b1b;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">Returned Device</div>';
                h+='<table style="width:100%;border-collapse:collapse;font-family:var(--hm-font)">';
                h+='<thead><tr style="border-bottom:1px solid #fecaca">';
                h+='<th style="text-align:left;padding:4px 0;font-size:10px;font-weight:700;color:#b91c1c;text-transform:uppercase;letter-spacing:.6px">Item</th>';
                h+='<th style="text-align:center;padding:4px 8px;font-size:10px;font-weight:700;color:#b91c1c;text-transform:uppercase;width:40px">Qty</th>';
                h+='<th style="text-align:right;padding:4px 0;font-size:10px;font-weight:700;color:#b91c1c;text-transform:uppercase;width:80px">Amount</th>';
                h+='</tr></thead><tbody>';

                // Show original items crossed out
                if(exch.original_items&&exch.original_items.length){
                    exch.original_items.forEach(function(oi){
                        h+='<tr style="border-bottom:1px solid #fee2e2">';
                        h+='<td style="padding:8px 0;text-decoration:line-through;color:#b91c1c;font-size:13px;opacity:.7">'+esc(oi.product_name||'Unknown')+(oi.ear_side?' <span style="font-size:11px;color:#dc2626">('+esc(oi.ear_side)+')</span>':'')+'</td>';
                        h+='<td style="text-align:center;padding:8px;text-decoration:line-through;color:#b91c1c;font-size:13px;opacity:.7">'+oi.quantity+'</td>';
                        h+='<td style="text-align:right;padding:8px 0;color:#dc2626;font-size:13px;font-weight:600">−€'+parseFloat(oi.unit_price||0).toFixed(2)+'</td>';
                        h+='</tr>';
                    });
                } else {
                    h+='<tr><td style="padding:8px 0;text-decoration:line-through;color:#b91c1c;font-size:13px;opacity:.7">'+esc(exch.device_name||'Original Device')+'</td>';
                    h+='<td style="text-align:center;padding:8px;text-decoration:line-through;color:#b91c1c;font-size:13px;opacity:.7">'+originalQty+'</td>';
                    h+='<td style="text-align:right;padding:8px 0;color:#dc2626;font-size:13px;font-weight:600">−€'+creditAmount.toFixed(2)+'</td></tr>';
                }
                h+='</tbody></table></div>';

                // Line items table — new items
                h+='<div style="flex:1;overflow-y:auto;margin-bottom:16px">';
                h+='<div style="font-size:10px;font-weight:700;color:var(--hm-text-light,#64748b);text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">New Device</div>';
                h+='<table style="width:100%;border-collapse:collapse;font-family:var(--hm-font)">';
                h+='<thead><tr style="border-bottom:2px solid var(--hm-navy,#151B33)">';
                h+='<th style="text-align:left;padding:6px 0;font-size:10px;font-weight:700;color:var(--hm-text-light,#64748b);text-transform:uppercase;letter-spacing:.8px">Item</th>';
                h+='<th style="text-align:center;padding:6px 8px;font-size:10px;font-weight:700;color:var(--hm-text-light,#64748b);text-transform:uppercase;letter-spacing:.8px;width:40px">Qty</th>';
                ...
                h+='<th style="width:28px"></th>';
                h+='</tr></thead>';
                h+='<tbody id="hm-op-items"><tr><td colspan="5" style="text-align:center;padding:24px 0;color:var(--hm-text-muted,#94a3b8);font-size:13px;font-style:italic">Select new device from the left panel</td></tr></tbody>';
                h+='</table></div>';
                ...
                // New subtotal
                h+='<div style="'+TR+';color:var(--hm-text-light,#64748b)"><span>New Subtotal (excl. VAT)</span><span id="hm-op-sub" style="font-variant-numeric:tabular-nums">€0.00</span></div>';
                h+='<div style="'+TR+';color:var(--hm-text-light,#64748b)"><span>VAT</span><span id="hm-op-vat" style="font-variant-numeric:tabular-nums">€0.00</span></div>';
                ...
                // PRSI deductions — pre-checked from original
                h+='<div id="hm-op-prsi-wrap" style="margin-top:6px;padding-top:8px;border-top:1px dashed var(--hm-border-light,#e2e8f0)">';
                h+='<div style="'+TR+'">';
                h+='<label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;color:var(--hm-text,#334155)">';
                h+='<input type="checkbox" id="hm-op-prsi-l"'+(exch.prsi_left?' checked':'')+' style="accent-color:#0e7490;width:14px;height:14px"> PRSI Grant — Left ear</label>';
                h+='<span style="font-size:13px;color:#059669;font-variant-numeric:tabular-nums">−€500.00</span></div>';
                h+='<div style="'+TR+'">';
                h+='<label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;color:var(--hm-text,#334155)">';
                h+='<input type="checkbox" id="hm-op-prsi-r"'+(exch.prsi_right?' checked':'')+' style="accent-color:#0e7490;width:14px;height:14px"> PRSI Grant — Right ear</label>';
                h+='<span style="font-size:13px;color:#059669;font-variant-numeric:tabular-nums">−€500.00</span></div>';
                h+='</div>';

                // Credit applied
                h+='<div style="'+TR+';margin-top:6px;padding-top:8px;border-top:1px dashed var(--hm-border-light,#e2e8f0);color:#059669;font-weight:600">';
                h+='<span style="display:flex;align-items:center;gap:6px"><svg width="14" height="14" fill="none" stroke="#059669" stroke-width="2"><rect x="2" y="5" width="10" height="8" rx="1"/><path d="M2 8h10"/></svg> Credit from Exchange</span>';
                h+='<span id="hm-ex-credit-amt" style="font-variant-numeric:tabular-nums">−€'+creditAmount.toFixed(2)+'</span></div>';

                // Grand total (balance due)
                h+='<div style="display:flex;justify-content:space-between;align-items:baseline;margin-top:10px;padding-top:10px;border-top:2px solid var(--hm-navy,#151B33)">';
                h+='<span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--hm-navy,#151B33)">Balance Due</span>';
                h+='<span id="hm-op-total" style="font-size:22px;font-weight:700;color:var(--hm-navy,#151B33);font-variant-numeric:tabular-nums">€0.00</span></div>';

                // Step down surplus message
                h+='<div id="hm-ex-surplus" style="display:none;margin-top:8px;padding:10px 14px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;font-size:12px;color:#059669;font-weight:600"></div>';

                h+='</div>'; // end totals block

                // Notes
                h+='<div style="margin-top:14px"><textarea id="hm-op-notes" rows="2" placeholder="Exchange notes..." style="'+INP+';resize:vertical;font-size:12px;background:var(--hm-bg-alt,#f8fafc);border:1px solid var(--hm-border,#e2e8f0)"></textarea></div>';
                ...
                // Action buttons
                h+='<div id="hm-op-actions" style="display:flex;gap:10px;margin-top:14px">';
                h+='<button id="hm-ex-submit" style="flex:1;padding:12px;font-size:13px;font-weight:700;border-radius:6px;border:none;background:#f59e0b;color:#fff;cursor:pointer;font-family:var(--hm-font-btn);transition:all .15s">Complete Exchange</button>';
                h+='<button id="hm-ex-pay-btn" style="display:none;flex:1;padding:12px;font-size:13px;font-weight:700;border-radius:6px;border:none;background:#059669;color:#fff;cursor:pointer;font-family:var(--hm-font-btn);transition:all .15s">Complete &amp; Take Payment</button>';
                h+='</div>';

                // ── Payment section (hidden until needed) ──
                h+='<div id="hm-op-paysec" style="display:none;border-top:1px solid var(--hm-border,#e2e8f0);padding-top:16px;margin-top:12px">';
                h+='<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:14px">';
                h+='<span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#059669">Payment</span>';
                h+='<span style="font-size:11px;color:var(--hm-text-light,#64748b)">Balance: <strong id="hm-op-pay-due" style="color:#059669;font-size:13px">€0.00</strong></span></div>';
                h+='<input type="hidden" id="hm-op-pay-amt" value="0">';
                ...
                h+='<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--hm-text-light,#64748b);margin-bottom:3px"><span>Allocated</span><span id="hm-op-pay-alloc-text">€0.00 / €0.00</span></div>';
                h+='<div style="height:4px;background:var(--hm-border-light,#f1f5f9);border-radius:2px;overflow:hidden"><div id="hm-op-pay-alloc-fill" style="height:100%;background:#059669;border-radius:2px;width:0%;transition:width .2s"></div></div>';
                h+='<div id="hm-op-pay-alloc-warn" style="display:none;font-size:11px;color:#dc2626;margin-top:3px">⚠ Allocated amount exceeds balance</div>';
                h+='</div>';

                h+='<div id="hm-op-pay-err" style="color:#ef4444;font-size:12px;margin-bottom:8px"></div>';

                h+='<div style="display:flex;gap:8px">';
                h+='<button id="hm-ex-pay-cancel" style="flex:0 0 auto;padding:10px 16px;font-size:12px;font-weight:600;border-radius:6px;border:1px solid var(--hm-border,#e2e8f0);background:#fff;color:var(--hm-text,#334155);cursor:pointer;font-family:var(--hm-font-btn)">Cancel</button>';
                h+='<button id="hm-ex-pay-confirm" style="flex:1;padding:10px;font-size:13px;font-weight:700;border-radius:6px;border:none;background:#059669;color:#fff;cursor:pointer;font-family:var(--hm-font-btn);transition:all .15s">Confirm Payment &amp; Complete</button>';
                h+='</div></div>'; // end payment section
                ...
                // ═══════ HELPERS ═══════
                function renderItems(){
                    if(!orderItems.length){$('#hm-op-items').html('<tr><td colspan="5" style="text-align:center;padding:24px 0;color:var(--hm-text-muted,#94a3b8);font-size:13px;font-style:italic">Select new device from the left panel</td></tr>');return;}
                    var t='';
                    orderItems.forEach(function(it,idx){
                        var details=[];
                        if(it.ear)details.push(it.ear);
                        if(it.speaker_size)details.push('Speaker: '+it.speaker_size+(it.speaker_type?' '+it.speaker_type:''));
                        if(it.dome_size)details.push('Dome: '+it.dome_size+(it.dome_type?' '+it.dome_type:''));
                        if(it.from_stock&&it.serial_number)details.push('SN: '+it.serial_number);
                        t+='<tr style="border-bottom:1px solid var(--hm-border-light,#f1f5f9)">';
                        t+='<td style="padding:10px 0;vertical-align:top">';
                        t+='<div style="font-size:13px;font-weight:600;color:var(--hm-navy,#151B33)">'+esc(it.name)+'</div>';
                        if(details.length)t+='<div style="font-size:11px;color:var(--hm-text-light,#64748b);margin-top:2px">'+esc(details.join(' · '))+'</div>';
                        if(it.from_stock)t+='<div style="font-size:10px;font-weight:700;color:var(--hm-teal,#0BB4C4);margin-top:2px">FROM STOCK</div>';
                        t+='</td>';
                        t+='<td style="text-align:center;padding:10px 8px;font-size:13px;color:var(--hm-text,#334155);vertical-align:top;font-variant-numeric:tabular-nums">'+it.qty+'</td>';
                        t+='<td style="text-align:right;padding:10px 8px;font-size:13px;color:var(--hm-text,#334155);vertical-align:top;font-variant-numeric:tabular-nums">€'+it.unit_price.toFixed(2)+'</td>';
                        t+='<td style="text-align:right;padding:10px 0;font-size:13px;font-weight:600;color:var(--hm-navy,#151B33);vertical-align:top;font-variant-numeric:tabular-nums">€'+(it.unit_price*it.qty).toFixed(2)+'</td>';
                        t+='<td style="padding:10px 0 10px 6px;vertical-align:top"><button class="hm-ex-rem" data-idx="'+idx+'" style="border:none;background:none;color:#b91c1c;width:22px;height:22px;cursor:pointer;font-size:14px;font-weight:700;display:flex;align-items:center;justify-content:center;opacity:.5;transition:opacity .15s" onmouseenter="this.style.opacity=1" onmouseleave="this.style.opacity=.5">×</button></td>';
                        t+='</tr>';
                    });
                    $('#hm-op-items').html(t);
                }
                ...
                function updateTotals(){
                    ...
                    var prsi=($('#hm-op-prsi-l').is(':checked')?500:0)+($('#hm-op-prsi-r').is(':checked')?500:0);
                    var newGrandTotal=Math.max(0,grossSum-disc-prsi);
                    var balanceDue=Math.max(0,newGrandTotal-creditAmount);
                    var surplus=Math.max(0,creditAmount-newGrandTotal);

                    $('#hm-op-sub').text('€'+net.toFixed(2));
                    $('#hm-op-vat').text('€'+vat.toFixed(2));
                    $('#hm-op-disc-amt').text('−€'+disc.toFixed(2));
                    $('#hm-op-total').text('€'+balanceDue.toFixed(2));
                    $('#hm-op-pay-due').text('€'+balanceDue.toFixed(2));
                    $('#hm-op-pay-amt').val(balanceDue.toFixed(2));

                    // Update balance due colour
                    if(balanceDue>0.01){
                        $('#hm-op-total').css('color','#dc2626');
                        $('#hm-ex-pay-btn').show();
                    } else {
                        $('#hm-op-total').css('color','#059669');
                        $('#hm-ex-pay-btn').hide();
                    }

                    // Surplus message
                    if(surplus>0.01){
                        $('#hm-ex-surplus').show().html('Patient has €'+surplus.toFixed(2)+' surplus credit — will remain on their account as credit.');
                    } else {
                        $('#hm-ex-surplus').hide();
                    }

                    // Update pay row default
                    if(balanceDue>0.01){
                        $('#hm-op-pay-methods .hm-op-pay-row').first().find('.hm-op-pay-row-amt').val(balanceDue.toFixed(2));
                    }
                }

                function _countProductQty(){
                    var q=0;
                    orderItems.forEach(function(it){if(it.type==='product')q+=it.qty;});
                    return q;
                }
                ...
                function _renderStockItems(items){
                    ...
                    items.forEach(function(si){
                        ...
                        html+='<div style="font-size:11px;color:var(--hm-text-light,#64748b);margin-top:2px">'+esc(si.style||'—')+' · '+esc(si.technology_level||'—')+' · SN: '+esc(si.serial_number||'N/A')+'</div>';
                        html+='</div>';
                        html+='<button type="button" class="hm-ex-stock-add" style="flex-shrink:0;font-size:11px;font-weight:700;padding:5px 14px;border-radius:6px;border:1.5px solid var(--hm-teal,#0BB4C4);background:#f0fdfa;color:var(--hm-teal,#0BB4C4);cursor:pointer;font-family:var(--hm-font-btn);white-space:nowrap;transition:all .15s">Add</button>';
                        html+='</div>';
                    });
                    if(!html)html='<div style="text-align:center;padding:20px;color:var(--hm-text-muted,#94a3b8);font-size:12px">No available stock found</div>';
                    $('#hm-op-stock-list').html(html);
                ...
                // ── Back button → Patient profile ──
                $(document).off('click.exback').on('click.exback','#hm-ex-back',function(){
                    // Go back to patient profile
                    window.location.href='/patients/?id='+patientId;
                });

                // ── Category change ──
                $(document).off('change.excat').on('change.excat','#hm-op-cat',function(){
                    var cat=$(this).val();
                    $('#hm-op-mfr-wrap,#hm-op-style-wrap,#hm-op-prod-wrap,#hm-op-tech-wrap,#hm-op-ear-wrap,#hm-op-add-wrap,#hm-op-charger-wrap,#hm-op-speaker-wrap,#hm-op-dome-wrap').hide();
                    $('#hm-op-mfr,#hm-op-style,#hm-op-prod').val('');
                    $('#hm-op-charger').prop('checked',false);
                ...
                // ── Browse mode toggle ──
                $(document).off('click.exbrowse').on('click.exbrowse','.hm-op-browse-btn',function(){
                    browseMode=$(this).data('mode');selectedRangeId=null;
                    $('.hm-op-browse-btn').css({background:'#fff',color:'var(--hm-navy,#151B33)'});
                    $(this).css({background:'var(--hm-navy,#151B33)',color:'#fff'});
                    if(browseMode==='range'){$('#hm-op-range-wrap').show();_populateRangeDropdown();}
                ...
                // ── Range change ──
                $(document).off('change.exrange').on('change.exrange','#hm-op-range',function(){
                    selectedRangeId=$(this).val()?parseInt($(this).val()):null;
                    _populateManufacturerDropdown();
                    $('#hm-op-style-wrap,#hm-op-prod-wrap,#hm-op-tech-wrap,#hm-op-ear-wrap,#hm-op-add-wrap').hide();
                    $('#hm-op-style,#hm-op-prod').val('');selectedProduct=null;
                ...
                // ── Manufacturer change ──
                $(document).off('change.exmfr').on('change.exmfr','#hm-op-mfr',function(){
                    var mfr=$(this).val();
                    $('#hm-op-style-wrap,#hm-op-prod-wrap,#hm-op-tech-wrap,#hm-op-ear-wrap,#hm-op-add-wrap').hide();
                    $('#hm-op-style,#hm-op-prod').val('');selectedProduct=null;
                    if(!mfr)return;
                ...
                // ── Style change ──
                $(document).off('change.exstyle').on('change.exstyle','#hm-op-style',function(){
                    var style=$(this).val(),mfr=$('#hm-op-mfr').val();
                    $('#hm-op-prod-wrap,#hm-op-tech-wrap,#hm-op-ear-wrap,#hm-op-add-wrap').hide();
                    $('#hm-op-prod').val('');selectedProduct=null;
                    if(!style)return;
                ...
                // ── Product select ──
                $(document).off('change.exprod').on('change.exprod','#hm-op-prod',function(){
                    var val=$(this).val();
                    if(!val){$('#hm-op-tech-wrap,#hm-op-ear-wrap,#hm-op-add-wrap').hide();selectedProduct=null;return;}
                    var opt=$(this).find('option:selected');
                    var cat=$('#hm-op-cat').val();
                ...
                // ── Add to exchange ──
                $(document).off('click.exadd').on('click.exadd','#hm-op-add-item',function(){
                    if(!selectedProduct){$('#hm-op-err').text('Select a product first.');return;}
                    var cat=$('#hm-op-cat').val();
                    var ear=$('#hm-op-ear').val()||'';
                    if(cat==='product'&&!ear){$('#hm-op-err').text('Please select which ear.');return;}
                    // Check qty limit for products
                    if(cat==='product'){
                        var pendingQty=ear==='Binaural'?2:1;
                        if(_countProductQty()+pendingQty>originalQty){
                            $('#hm-op-err').text('Exchange allows '+originalQty+' aid'+(originalQty>1?'s':'')+'. You have already added '+_countProductQty()+'.');
                            return;
                        }
                    }
                    $('#hm-op-err').text('');
                    ...
                    selectedProduct=null;
                    $('#hm-op-cat').val('').trigger('change.excat');
                });

                // ── Remove item ──
                $(document).off('click.exrem').on('click.exrem','.hm-ex-rem',function(){
                    var idx=parseInt($(this).data('idx'));
                    var removed=orderItems[idx];
                    orderItems.splice(idx,1);
                    if(removed&&removed.from_stock)hasFromStock=orderItems.some(function(it){return it.from_stock;});
                    renderItems();updateTotals();
                });

                // ── Stock picker ──
                $(document).off('click.exstockbtn').on('click.exstockbtn','#hm-op-stock-btn',function(){
                    var $panel=$('#hm-op-stock-panel');
                    if($panel.is(':visible')){$panel.slideUp(200);return;}
                    $panel.slideDown(200);
                    var $list=$('#hm-op-stock-list');
                    $list.html('<div style="text-align:center;padding:20px;color:var(--hm-text-muted,#94a3b8);font-size:12px">Loading stock…</div>');
                    $.post(HM.ajax_url,{action:'hm_stock_hearing_aids',nonce:HM.nonce,status:'Available'},function(r){
                        if(!r||!r.success){$list.html('<div style="text-align:center;padding:20px;color:#ef4444;font-size:12px">Failed to load stock</div>');return;}
                        _renderStockItems(r.data||[]);
                    });
                });

                $(document).off('click.exstockclose').on('click.exstockclose','#hm-op-stock-close',function(){
                    $('#hm-op-stock-panel').slideUp(200);
                });

                $(document).off('input.exstocksearch').on('input.exstocksearch','#hm-op-stock-search',function(){
                    var q=$(this).val().toLowerCase();
                    $('#hm-op-stock-list .hm-stock-item').each(function(){
                        $(this).toggle($(this).text().toLowerCase().indexOf(q)>-1);
                    });
                ...
                $(document).off('click.exstockpick').on('click.exstockpick','.hm-ex-stock-add',function(e){
                    e.stopPropagation();
                    // Check qty limit
                    if(_countProductQty()+1>originalQty){
                        self.toast('Exchange allows '+originalQty+' aid'+(originalQty>1?'s':'')+'. Already added '+_countProductQty()+'.','warning');
                        return;
                    }
                    var $card=$(this).closest('.hm-stock-item');
                    var stockId=$card.data('sid');
                    ...
                    if(!product)self.toast('No matching product in catalog — price set to €0','warning');
                ...
                // ── PRSI changes ──
                $(document).off('change.exprsi').on('change.exprsi','#hm-op-prsi-l,#hm-op-prsi-r',updateTotals);

                // ── Discount ──
                $(document).off('input.exdisc').on('input.exdisc','#hm-op-disc',function(){updateTotals();});
                $(document).off('click.exdiscmode').on('click.exdiscmode','.hm-op-disc-mode',function(){
                    var mode=$(this).data('mode');discountMode=mode;
                    $('.hm-op-disc-mode').css({background:'#fff',color:'var(--hm-text,#334155)'});
                    $(this).css({background:'var(--hm-navy,#151B33)',color:'#fff'});
                    if(mode==='pct'){$('#hm-op-disc-unit').text('%');$('#hm-op-disc').attr({max:100,step:1});}
                ...
                // ── Complete Exchange (no additional payment needed) ──
                $(document).off('click.exsubmit').on('click.exsubmit','#hm-ex-submit',function(){
                    if(!orderItems.length){$('#hm-op-err').text('Please select a new device.');return;}
                    var balanceDue=parseFloat($('#hm-op-pay-amt').val())||0;
                    if(balanceDue>0.01){
                        // Balance due — show payment section instead
                        _showPaymentSection();
                        return;
                    }
                    var $btn=$(this);$btn.prop('disabled',true).text('Processing…');
                ...
                $(document).off('click.expaybtn').on('click.expaybtn','#hm-ex-pay-btn',function(){
                    if(!orderItems.length){$('#hm-op-err').text('Please select a new device.');return;}
                    _showPaymentSection();
                });

                function _showPaymentSection(){
                    var balanceDue=parseFloat($('#hm-op-pay-amt').val())||0;
                    $('#hm-op-pay-due').text('€'+balanceDue.toFixed(2));
                    $('#hm-op-pay-methods .hm-op-pay-row').first().find('.hm-op-pay-row-amt').val(balanceDue.toFixed(2));
                    _updatePayAlloc();
                ...
                // ── Cancel payment ──
                $(document).off('click.expaycancel').on('click.expaycancel','#hm-ex-pay-cancel',function(){
                    $('#hm-op-paysec').slideUp(200);$('#hm-op-actions').show();
                    var $methods=$('#hm-op-pay-methods');
                    $methods.find('.hm-op-pay-row').slice(1).remove();
                    $methods.find('.hm-op-pay-row').first().find('.hm-op-pay-rem-row').remove();
                    $methods.find('.hm-op-pay-row-method').val('');$methods.find('.hm-op-pay-row-amt').val('');
                });

                // ── Payment allocation recalc ──
                $(document).off('input.expayrowamt').on('input.expayrowamt','.hm-op-pay-row-amt',function(){
                    var $rows=$('#hm-op-pay-methods .hm-op-pay-row');
                    if($rows.length<2){_updatePayAlloc();return;}
                    var $this=$(this),$lastRow=$rows.last(),$lastAmt=$lastRow.find('.hm-op-pay-row-amt');
                    if(!$this.is($lastAmt)){
                        var total=parseFloat($('#hm-op-pay-amt').val())||0;
                        var allocated=0;
                        $rows.each(function(){var $r=$(this);if(!$r.is($lastRow)){allocated+=parseFloat($r.find('.hm-op-pay-row-amt').val())||0;}});
                        var rem=Math.round((total-allocated)*100)/100;
                        $lastAmt.val(rem>0?rem.toFixed(2):'0.00');
                    }
                    _updatePayAlloc();
                ...
                // ── Add payment method ──
                $(document).off('click.exaddmethod').on('click.exaddmethod','#hm-op-pay-add-method',function(){
                    var total=parseFloat($('#hm-op-pay-amt').val())||0;
                    var allocated=0;$('#hm-op-pay-methods .hm-op-pay-row-amt').each(function(){allocated+=parseFloat($(this).val())||0;});
                    var rem=Math.round((total-allocated)*100)/100;
                    var row='<div class="hm-op-pay-row" style="display:flex;gap:8px;align-items:center;margin-bottom:8px">';
                    row+='<select class="hm-op-pay-row-method" style="'+INP+';flex:1"><option value="">— Method —</option><option value="Card">Card</option><option value="Cash">Cash</option><option value="Bank Transfer">Bank Transfer</option><option value="Cheque">Cheque</option></select>';
                    row+='<div style="position:relative;width:110px;flex-shrink:0"><span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);font-size:12px;color:var(--hm-text-light,#64748b);pointer-events:none">€</span>';
                    row+='<input type="number" class="hm-op-pay-row-amt" step="0.01" min="0" value="'+(rem>0?rem.toFixed(2):'0.00')+'" style="'+INP+';padding-left:22px;text-align:right;font-weight:600;font-variant-numeric:tabular-nums"></div>';
                    row+='<button type="button" class="hm-ex-pay-rem-row" style="border:none;background:none;color:#b91c1c;font-size:16px;cursor:pointer;padding:4px;opacity:.5">×</button>';
                    row+='</div>';
                    $('#hm-op-pay-methods').append(row);
                    if(!$('#hm-op-pay-methods .hm-op-pay-row').first().find('.hm-ex-pay-rem-row').length){
                        $('#hm-op-pay-methods .hm-op-pay-row').first().append('<button type="button" class="hm-ex-pay-rem-row" style="border:none;background:none;color:#b91c1c;font-size:16px;cursor:pointer;padding:4px;opacity:.5">×</button>');
                    }
                    _updatePayAlloc();
                });

                // ── Remove payment method ──
                $(document).off('click.exremmethod').on('click.exremmethod','.hm-ex-pay-rem-row',function(){
                    var $rows=$('#hm-op-pay-methods .hm-op-pay-row');
                    if($rows.length<=1)return;
                    $(this).closest('.hm-op-pay-row').remove();
                    $rows=$('#hm-op-pay-methods .hm-op-pay-row');
                    if($rows.length===1)$rows.find('.hm-ex-pay-rem-row').remove();
                    var total=parseFloat($('#hm-op-pay-amt').val())||0;
                    if($rows.length===1)$rows.find('.hm-op-pay-row-amt').val(total.toFixed(2));
                    _updatePayAlloc();
                });
                ...
                $(document).off('click.expayconfirm').on('click.expayconfirm','#hm-ex-pay-confirm',function(){
                    if(!orderItems.length){$('#hm-op-pay-err').text('Please select a new device.');return;}
                    var totalDue=parseFloat($('#hm-op-pay-amt').val())||0;
                    var $rows=$('#hm-op-pay-methods .hm-op-pay-row');
                    var splitPayload=[];var allocated=0;var valid=true;
                    $rows.each(function(i){
                        var m=$(this).find('.hm-op-pay-row-method').val();
                        var ea=parseFloat($(this).find('.hm-op-pay-row-amt').val())||0;
                        if(!m){$('#hm-op-pay-err').text('Select a payment method for row '+(i+1)+'.');valid=false;return false;}
                        if(ea<=0){$('#hm-op-pay-err').text('Enter a valid amount for row '+(i+1)+'.');valid=false;return false;}
                        splitPayload.push({method:m,amount:ea});allocated+=ea;
                    });
                    if(!valid)return;
                    allocated=Math.round(allocated*100)/100;
                    if(Math.abs(allocated-totalDue)>0.01){$('#hm-op-pay-err').text('Payment must equal €'+totalDue.toFixed(2)+'.');return;}

                    var $btn=$(this);$btn.prop('disabled',true).text('Processing…');
                    _submitExchange($btn,splitPayload);
                });

                // ── Core submit function ──
                function _submitExchange($btn,paymentData){
                    var discVal=parseFloat($('#hm-op-disc').val())||0;
                    var items=orderItems.map(function(it){
                        return{product_id:it.id,type:it.type,name:it.name,unit_price:it.unit_price,
                            qty:it.qty,ear:it.ear||'',vat_rate:it.vat_rate,vat_amount:it.vat_amount,
                            stock_id:it.stock_id||null,serial_number:it.serial_number||'',from_stock:!!it.from_stock,
                            speaker_size:it.speaker_size||'',speaker_type:it.speaker_type||'',
                            dome_size:it.dome_size||'',dome_type:it.dome_type||''};
                    });

                    $.post(HM.ajax_url,{
                        action:'hm_complete_exchange',nonce:HM.nonce,
                        exchange_id:exchId,
                        items_json:JSON.stringify(items),
                        notes:$('#hm-op-notes').val()||'',
                        prsi_left:$('#hm-op-prsi-l').is(':checked')?1:0,
                        prsi_right:$('#hm-op-prsi-r').is(':checked')?1:0,
                        discount_pct:discountMode==='pct'?discVal:0,
                        discount_euro:discountMode==='eur'?discVal:0,
                        from_stock:hasFromStock||orderItems.some(function(it){return it.from_stock;})?1:0
                    },function(r){
                        if(!r||!r.success){
                            $btn.prop('disabled',false).text($btn.attr('id')==='hm-ex-submit'?'Complete Exchange':'Confirm Payment & Complete');
                            $('#hm-op-err,#hm-op-pay-err').text(r&&r.data?r.data:'Exchange failed');
                            return;
                        }
                        var d=r.data;
                        var balanceDue=parseFloat(d.balance_due)||0;

                        // If payment was provided, record it
                        if(paymentData&&paymentData.length&&balanceDue>0.01){
                            post('record_order_payment',{
                                order_id:d.new_order_id,order_number:d.order_number,
                                amount:paymentData.reduce(function(s,p){return s+p.amount;},0),
                                payment_method:paymentData[0].method,
                                split_payments_json:paymentData.length>1?JSON.stringify(paymentData):'[]'
                            }).then(function(pr){
                                if(pr.success){
                                    self.toast('Exchange completed! '+d.exchange_type.replace('_',' ')+'. Payment of €'+paymentData.reduce(function(s,p){return s+p.amount;},0).toFixed(2)+' recorded.');
                                } else {
                                    self.toast('Exchange completed but payment failed: '+(pr.data&&pr.data.message?pr.data.message:'Unknown error'),'warning');
                                }
                                _showCompletionSummary(d);
                            }).fail(function(){
                                self.toast('Exchange completed but payment request failed','warning');
                                _showCompletionSummary(d);
                            });
                        } else {
                            var msg='Exchange completed! ';
                            if(d.exchange_type==='step_down')msg+='Step down — surplus credit on account.';
                            else if(d.exchange_type==='same_value')msg+='Same value transfer — no balance due.';
                            else if(d.exchange_type==='step_up'&&balanceDue<=0.01)msg+='Fully covered by credit.';
                            else msg+='Credit applied: €'+parseFloat(d.credit_applied||0).toFixed(2);
                            self.toast(msg);
                            _showCompletionSummary(d);
                        }
                    }).fail(function(){
                        $btn.prop('disabled',false).text($btn.attr('id')==='hm-ex-submit'?'Complete Exchange':'Confirm Payment & Complete');
                        $('#hm-op-err,#hm-op-pay-err').text('Network error');
                    });
                }

                // ── Show completion summary ──
                function _showCompletionSummary(d){
                    var s='<div style="text-align:center;padding:40px 20px">';
                    s+='<div style="width:60px;height:60px;border-radius:50%;background:#f0fdf4;display:flex;align-items:center;justify-content:center;margin:0 auto 16px">';
                    s+='<svg width="32" height="32" fill="none" stroke="#059669" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11L9.5 22 4 17"/></svg></div>';
                    s+='<div style="font-size:20px;font-weight:700;color:var(--hm-navy,#151B33);font-family:var(--hm-font-title);margin-bottom:8px">Exchange Complete</div>';
                    s+='<div style="font-size:13px;color:var(--hm-text-light,#64748b);margin-bottom:20px">'+esc(exch.exchange_number||'')+'</div>';

                    s+='<div style="display:inline-block;text-align:left;background:var(--hm-bg-alt,#f8fafc);border-radius:10px;padding:16px 24px;max-width:360px;width:100%">';
                    s+='<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0"><span style="color:var(--hm-text-light,#64748b)">New Order</span><span style="font-weight:600;color:var(--hm-navy,#151B33)">'+esc(d.order_number||'')+'</span></div>';
                    s+='<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0"><span style="color:var(--hm-text-light,#64748b)">New Invoice</span><span style="font-weight:600;color:var(--hm-navy,#151B33)">'+esc(d.invoice_number||'')+'</span></div>';
                    s+='<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0"><span style="color:var(--hm-text-light,#64748b)">Credit Note</span><span style="font-weight:600;color:#059669">'+esc(d.credit_note_number||'')+'</span></div>';
                    s+='<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;border-top:1px solid var(--hm-border,#e2e8f0);margin-top:4px;padding-top:8px"><span style="color:var(--hm-text-light,#64748b)">Exchange Type</span><span style="font-weight:600;color:'+(d.exchange_type==='step_up'?'#dc2626':d.exchange_type==='step_down'?'#059669':'#f59e0b')+'">'+esc((d.exchange_type||'').replace('_',' ').toUpperCase())+'</span></div>';
                    s+='<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0"><span style="color:var(--hm-text-light,#64748b)">Credit Applied</span><span style="font-weight:600;color:#059669">€'+parseFloat(d.credit_applied||0).toFixed(2)+'</span></div>';
                    var bal=parseFloat(d.balance_due||0);
                    if(bal>0.01)s+='<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0"><span style="color:var(--hm-text-light,#64748b)">Balance Due</span><span style="font-weight:700;color:#dc2626">€'+bal.toFixed(2)+'</span></div>';
                    s+='</div>';

                    s+='<div style="margin-top:24px;display:flex;gap:10px;justify-content:center">';
                    s+='<button onclick="window.location.href=\'/patients/?id='+patientId+'\'" style="padding:10px 20px;font-size:13px;font-weight:700;border-radius:6px;border:none;background:var(--hm-teal,#0BB4C4);color:#fff;cursor:pointer;font-family:var(--hm-font-btn)">Back to Patient</button>';
                    s+='<button onclick="window.location.href=\'/calendar/\'" style="padding:10px 20px;font-size:13px;font-weight:600;border-radius:6px;border:1px solid var(--hm-border,#e2e8f0);background:#fff;color:var(--hm-text,#334155);cursor:pointer;font-family:var(--hm-font-btn)">Go to Calendar</button>';
                    s+='</div></div>';
