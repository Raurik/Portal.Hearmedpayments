...
class HearMed_Accounting {
    ...
    public static function render_dashboard() {
        ...
        <?php if(isset($_GET['qbo_connected'])):?><div class="hm-notice hm-notice--success" style="margin-bottom:1.5rem;">✓ QuickBooks connected! Invoices and bills will now sync automatically.</div><?php endif;?>
        <?php if($qf>0):?><div class="hm-notice hm-notice--warning" style="margin-bottom:1.5rem;">⚠ <?php echo $qf;?> QuickBooks sync failure(s) this week. <a href="<?php echo esc_url($base.'?hm_action=qbo');?>">View and retry →</a></div><?php endif;?>
        <div class="hm-kpi-grid">
            <?php $cards=[['Today\'s Revenue','€'.number_format($tr,2),'',''],['This Month','€'.number_format($mr,2),'',''],['This Year','€'.number_format($yr,2),'',''],['Outstanding','€'.number_format($out,2),$out>0?'hm-kpi-card--warning':'',$base.'?hm_action=invoices&status=unpaid'],['Awaiting Approval',$ow,$ow>0?'hm-kpi-card--alert':'',HearMed_Utils::page_url('orders').'?status=Awaiting+Approval'],['Awaiting Fitting',$fq,$fq>0?'hm-kpi-card--info':'',HearMed_Utils::page_url('orders').'?status=Awaiting+Fitting'],['Unpaid Supplier Bills',$su,$su>0?'hm-kpi-card--warning':'',$base.'?hm_action=supplier']];
        ...
        <div class="hm-accounting-grid">
            ...
            <div class="hm-card">
                ...
                <?php foreach($by_clinic as $r):?><tr><td><?php echo esc_html($r->clinic_name);?></td><td class="hm-money">€<?php echo number_format($r->total,2);?></td></tr><?php endforeach;?>
                </tbody></table>
        ...
        <div class="hm-card" style="margin-top:1.5rem;">
            ...
            <table class="hm-table">
                ...
                <tbody>
                <?php if(empty($recent)):?><tr><td colspan="6" class="hm-table__empty">No payments yet.</td></tr>
    ...
    public static function render_invoices() {
        ...
        $invoices=$db->get_results("SELECT inv.id,inv.invoice_number,inv.invoice_date,inv.grand_total,inv.balance_remaining,inv.payment_status,inv.quickbooks_id,inv.qbo_sync_status,p.first_name,p.last_name,c.clinic_name,o.order_number FROM hearmed_core.invoices inv JOIN hearmed_core.patients p ON p.id=inv.patient_id JOIN hearmed_reference.clinics c ON c.id=inv.clinic_id LEFT JOIN hearmed_core.orders o ON o.invoice_id=inv.id {$where} ORDER BY inv.invoice_date DESC LIMIT 200",$params);
        ob_start();?>
        <div class="hm-content">
        <div class="hm-page-header"><a href="<?php echo esc_url($base);?>" class="hm-back-btn">← Accounting</a><h1 class="hm-page-title">Invoices</h1></div>
        <div class="hm-tabs">
            <?php foreach(['all'=>'All','unpaid'=>'Unpaid','Paid'=>'Paid','Partially Paid'=>'Part Paid'] as $k=>$l):?>
            <a href="<?php echo esc_url($base.'?hm_action=invoices&status='.$k);?>" class="hm-tab <?php echo $status===$k?'hm-tab--active':'';?>"><?php echo esc_html($l);?></a>
            <?php endforeach;?>
        </div>
        <table class="hm-table">
            ...
            <tbody>
            <?php if(empty($invoices)):?><tr><td colspan="9" class="hm-table__empty">No invoices found.</td></tr>
            ...
            <tr>
                ...
                <td class="hm-money <?php echo floatval($inv->balance_remaining)>0?'hm-text--orange':'hm-text--green';?>">€<?php echo number_format($inv->balance_remaining,2);?></td>
                <td><?php echo self::payment_badge($inv->payment_status);?></td>
                <td>
                    <?php if($inv->quickbooks_id):?><span class="hm-badge hm-badge--green">Synced</span>
                    <?php elseif($inv->qbo_sync_status==='failed'):?><span class="hm-badge hm-badge--red">Failed</span><button class="hm-btn hm-btn--sm hm-btn--ghost hm-retry-qbo" data-invoice-id="<?php echo $inv->id;?>" data-nonce="<?php echo wp_create_nonce('hearmed_nonce');?>">Retry</button>
                    <?php else:?><span class="hm-badge hm-badge--grey">Pending</span><?php endif;?>
                </td>
            </tr>
            <?php endforeach; endif;?>
            </tbody>
        ...
        <script>document.querySelectorAll('.hm-retry-qbo').forEach(btn=>{btn.addEventListener('click',function(){this.textContent='...';fetch('<?php echo admin_url('admin-ajax.php');?>',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'hm_retry_qbo_sync',invoice_id:this.dataset.invoiceId,nonce:this.dataset.nonce})}).then(r=>r.json()).then(d=>{if(d.success){this.closest('td').innerHTML='<span class="hm-badge hm-badge--green">Synced</span>';}else{this.textContent='Failed';}});});});</script>
        <?php return ob_get_clean();
    }

    public static function render_supplier_list() {
        $db=HearMed_DB::instance();$base=HearMed_Utils::page_url('accounting');
        $bills=$db->get_results("SELECT * FROM hearmed_core.supplier_invoices ORDER BY invoice_date DESC LIMIT 100",[]);
        ob_start();?>
        <div class="hm-content">
        <div class="hm-page-header"><a href="<?php echo esc_url($base);?>" class="hm-back-btn">← Accounting</a><h1 class="hm-page-title">Supplier Invoices</h1><a href="<?php echo esc_url($base.'?hm_action=supplier-new');?>" class="hm-btn hm-btn--primary">+ Add Invoice</a></div>
        <table class="hm-table">
            <thead><tr><th>Ref</th><th>Supplier</th><th>Date</th><th>Due</th><th>Total</th><th>Paid?</th><th>QBO</th></tr></thead>
            <tbody>
            <?php if(empty($bills)):?><tr><td colspan="7" class="hm-table__empty">No supplier invoices yet.</td></tr>
            ...
            <tr>
                ...
                <td class="<?php echo $od?'hm-text--red':'hm-muted';?>"><?php echo date('d M Y',strtotime($b->due_date));?><?php echo $od?' ⚠':'';?></td>
                <td class="hm-money">€<?php echo number_format($b->total_amount,2);?></td>
                <td><?php echo self::payment_badge($b->payment_status??'unpaid');?></td>
                <td><?php if(!empty($b->qbo_bill_id)):?><span class="hm-badge hm-badge--green">Synced</span><?php elseif(($b->qbo_sync_status??'')==='failed'):?><span class="hm-badge hm-badge--red">Failed</span><?php else:?><span class="hm-badge hm-badge--grey">Pending</span><?php endif;?></td>
            </tr>
            <?php endforeach; endif;?>
            </tbody>
        </table>
    ...
    public static function render_supplier_form() {
        $base=HearMed_Utils::page_url('accounting');$nonce=wp_create_nonce('hearmed_nonce');
        $accounts=HearMed_DB::get_results("SELECT qbo_account_id,account_name FROM hearmed_admin.qbo_accounts WHERE account_type IN ('Expense','Cost of Goods Sold','Other Expense') ORDER BY account_name",[]);
        $opts='<option value="">— Account —</option>';
        foreach($accounts as $a) $opts.='<option value="'.esc_attr($a->qbo_account_id).'">'.esc_html($a->account_name).'</option>';
        ob_start();?>
        <div class="hm-content">
        <div class="hm-page-header"><a href="<?php echo esc_url($base.'?hm_action=supplier');?>" class="hm-back-btn">← Supplier Invoices</a><h1 class="hm-page-title">New Supplier Invoice</h1><span class="hm-workflow-hint">Saves to portal + pushes to QuickBooks as a Bill.</span></div>
        <form id="hm-sup-form" class="hm-form hm-card" style="max-width:820px;">
            ...
            <div class="hm-form__row">
                <div class="hm-form__field"><label class="hm-label">Supplier Name <span class="hm-required">*</span></label><input type="text" name="supplier_name" class="hm-input" required placeholder="e.g. Phonak Ireland"></div>
                <div class="hm-form__field"><label class="hm-label">Invoice Reference <span class="hm-required">*</span></label><input type="text" name="supplier_invoice_ref" class="hm-input" required placeholder="e.g. INV-2026-0042"></div>
            </div>
            <div class="hm-form__row">
                <div class="hm-form__field"><label class="hm-label">Invoice Date</label><input type="date" name="invoice_date" class="hm-input" value="<?php echo date('Y-m-d');?>"></div>
                <div class="hm-form__field"><label class="hm-label">Due Date</label><input type="date" name="due_date" class="hm-input" value="<?php echo date('Y-m-d',strtotime('+30 days'));?>"></div>
                <div class="hm-form__field"><label class="hm-label">Currency</label><select name="currency" class="hm-input"><option value="EUR">EUR</option><option value="GBP">GBP</option><option value="USD">USD</option></select></div>
            </div>
            ...
            <div class="hm-form__field" style="margin-top:1rem;"><label class="hm-label">Notes</label><textarea name="notes" class="hm-input hm-input--textarea" rows="2"></textarea></div>
            <div class="hm-form__actions"><a href="<?php echo esc_url($base.'?hm_action=supplier');?>" class="hm-btn hm-btn--ghost">Cancel</a><button type="submit" class="hm-btn hm-btn--primary">Save &amp; Push to QuickBooks →</button></div>
        ...
        (function(){
            ...
            function newRow(){
                ...
                tr.innerHTML=`<td><input type="text" name="items[${idx}][description]" class="hm-input hm-input--sm" required placeholder="Description"></td><td><select name="items[${idx}][qbo_account_id]" class="hm-input hm-input--sm">${acctOpts}</select></td><td><input type="number" name="items[${idx}][qty]" class="hm-input hm-input--sm sup-qty" min="1" value="1"></td><td><input type="number" name="items[${idx}][unit_price]" class="hm-input hm-input--sm sup-price" step="0.01" value="0.00"></td><td><input type="number" name="items[${idx}][vat_rate]" class="hm-input hm-input--sm sup-vat" step="0.1" value="23"></td><td class="hm-money sup-line-total">€0.00</td><td><button type="button" class="hm-btn hm-btn--sm hm-btn--danger hm-remove-sup">✕</button></td>`;
                idx++; body.appendChild(tr); bind(tr);
            }
            function bind(tr){
                tr.querySelector('.hm-remove-sup').addEventListener('click',()=>{if(body.querySelectorAll('tr').length>1){tr.remove();recalc();}});
            ...
            function recalc(){
                ...
                body.querySelectorAll('tr').forEach(tr=>{
                    ...
                    const c=tr.querySelector('.sup-line-total');if(c)c.textContent='€'+tot.toFixed(2);
                    s+=ex;v+=vat;
                });
                const t=s+v;
            ...
            document.getElementById('hm-sup-form').addEventListener('submit',function(e){
                e.preventDefault();
                const btn=this.querySelector('[type=submit]');btn.disabled=true;btn.textContent='Saving...';
                fetch('<?php echo admin_url('admin-ajax.php');?>',{method:'POST',body:new URLSearchParams(new FormData(this))})
                .then(r=>r.json()).then(d=>{
                    const m=document.getElementById('hm-sup-msg');m.style.display='block';
                    if(d.success){m.className='hm-notice hm-notice--success';m.textContent=d.data.message;setTimeout(()=>window.location='<?php echo esc_url($base.'?hm_action=supplier');?>',1500);}
                    else{m.className='hm-notice hm-notice--error';m.textContent=d.data;btn.disabled=false;btn.textContent='Save & Push to QuickBooks →';}
                });
            });
        })();
        </script>
    ...
    public static function render_bank_feed() {
        $db=HearMed_DB::instance();$base=HearMed_Utils::page_url('accounting');$nonce=wp_create_nonce('hearmed_nonce');
        if(isset($_GET['refresh'])){HearMed_QBO::pull_bank_transactions(30);wp_redirect($base.'?hm_action=bank-feed');exit;}
        ...
        <div class="hm-page-header">
            ...
            <div class="hm-page-header__actions">
                <?php if($last):?><span class="hm-muted" style="font-size:.875rem;">Synced: <?php echo date('d M Y H:i',strtotime($last));?></span><?php endif;?>
            ...
            <tr class="<?php echo $t->assigned?'hm-muted':'';?>">
                ...
                <td>
                    ...
                    <select class="hm-input hm-input--sm hm-bank-acct" data-id="<?php echo $t->id;?>">
                        ...
                        <?php foreach($accounts as $a):?><option value="<?php echo esc_attr($a->qbo_account_id);?>" <?php echo $t->qbo_account_id===$a->qbo_account_id?'selected':'';?>><?php echo esc_html($a->account_name);?></option><?php endforeach;?>
                    </select>
                    <?php else:?><span class="hm-badge hm-badge--green">✓ Assigned</span><?php endif;?>
                </td>
                <td><?php if(!$t->assigned):?><button class="hm-btn hm-btn--sm hm-btn--primary hm-save-txn" data-id="<?php echo $t->id;?>" data-nonce="<?php echo esc_attr($nonce);?>">Save</button><?php endif;?></td>
            </tr>
            <?php endforeach;?>
            </tbody>
        </table>
        <script>document.querySelectorAll('.hm-save-txn').forEach(btn=>{btn.addEventListener('click',function(){const row=this.closest('tr');const acct=row.querySelector('.hm-bank-acct').value;if(!acct){alert('Select an account first.');return;}this.disabled=true;this.textContent='...';fetch('<?php echo admin_url('admin-ajax.php');?>',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'hm_assign_bank_txn',txn_id:this.dataset.id,account_id:acct,nonce:this.dataset.nonce})}).then(r=>r.json()).then(d=>{if(d.success){row.classList.add('hm-muted');row.cells[4].innerHTML='<span class="hm-badge hm-badge--green">✓ Assigned</span>';row.cells[5].innerHTML='';}else{alert('Error: '+d.data);this.disabled=false;this.textContent='Save';}});});});</script>
        <?php endif;?>
        </div>
        <?php return ob_get_clean();
    }
    ...
    public static function render_qbo_manager() {
        $base=HearMed_Utils::page_url('accounting');$status=HearMed_QBO::connection_status();$nonce=wp_create_nonce('hearmed_nonce');
        $log=HearMed_DB::get_results("SELECT * FROM hearmed_admin.qbo_sync_log ORDER BY created_at DESC LIMIT 20",[]);
        ob_start();?>
        <div class="hm-content">
        <div class="hm-page-header"><a href="<?php echo esc_url($base);?>" class="hm-back-btn">← Accounting</a><h1 class="hm-page-title">QuickBooks Connection</h1></div>
        <div class="hm-card" style="max-width:580px;">
            <?php if($status['connected']):?>
            <div class="hm-qbo-status hm-qbo-status--connected"><span class="hm-qbo-status__dot"></span><div><strong>Connected</strong><p class="hm-muted" style="margin:.25rem 0 0;">Company ID: <?php echo esc_html($status['realm_id']);?><br>Token valid until: <?php echo date('d M Y H:i',strtotime($status['expires_at']));?></p></div></div>
            ...
            <?php else:?>
            <div class="hm-qbo-status hm-qbo-status--disconnected"><span class="hm-qbo-status__dot"></span><div><strong>Not Connected</strong><p class="hm-muted">Connect to enable invoice sync, supplier bills, and bank feed.</p></div></div>
            <div style="margin-top:1.5rem;"><a href="<?php echo esc_url($status['auth_url']);?>" class="hm-btn hm-btn--primary hm-btn--block">Connect to QuickBooks Online →</a></div>
            <?php endif;?>
        </div>
        <div class="hm-card" style="margin-top:1.5rem;">
            ...
            <table class="hm-table">
                ...
                <tbody>
                <?php if(empty($log)):?><tr><td colspan="5" class="hm-table__empty">No sync activity yet.</td></tr>
                <?php else: foreach($log as $l):$bc=match($l->status){'success'=>'hm-badge--green','failed','error'=>'hm-badge--red',default=>'hm-badge--grey'};?>
                <tr>
        ...
        <script>(function(){const a='<?php echo admin_url('admin-ajax.php');?>';const sa=document.getElementById('hm-sync-accts');const disc=document.getElementById('hm-qbo-disc');if(sa){sa.addEventListener('click',function(){this.disabled=true;this.textContent='Syncing...';fetch(a,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'hm_qbo_sync_accounts',nonce:this.dataset.nonce})}).then(r=>r.json()).then(d=>{const m=document.getElementById('hm-qbo-msg');m.style.display='block';m.className='hm-notice '+(d.success?'hm-notice--success':'hm-notice--error');m.textContent=d.success?'✓ Accounts synced.':'Failed: '+d.data;this.disabled=false;this.textContent='↻ Sync Chart of Accounts';});});}if(disc){disc.addEventListener('click',function(){if(!confirm('Disconnect QuickBooks?'))return;fetch(a,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'hm_qbo_disconnect',nonce:this.dataset.nonce})}).then(r=>r.json()).then(d=>{if(d.success)location.reload();});});}})();</script>
        <?php return ob_get_clean();
    }
    ...
    public static function ajax_save_supplier_invoice() {
        ...
        $id=HearMed_DB::insert('hearmed_core.supplier_invoices',[
            'supplier_name'=>$sn,'supplier_invoice_ref'=>$sr,
            ...
            'payment_status'=>'unpaid','qbo_sync_status'=>'pending',
            'created_by'=>HearMed_Auth::current_user()->id??null,
            'created_at'=>date('Y-m-d H:i:s'),
        ]);
        if(!$id) wp_send_json_error('Failed to save. Please try again.');
        foreach($items as $item){
            $qty=intval($item['qty']??1);$price=floatval($item['unit_price']??0);$vatR=floatval($item['vat_rate']??0);
            $ex=$qty*$price;$vat=$ex*($vatR/100);
            HearMed_DB::insert('hearmed_core.supplier_invoice_items',[
                'supplier_invoice_id'=>$id,'description'=>sanitize_text_field($item['description']??''),
                'qbo_account_id'=>sanitize_text_field($item['qbo_account_id']??''),
                'quantity'=>$qty,'unit_price'=>$price,'vat_rate'=>$vatR,'vat_amount'=>$vat,'line_total'=>$ex+$vat,
            ]);
        }
    ...
    public static function ajax_retry_qbo_sync() {
        ...
        HearMed_QBO::retry_failed_invoice($order->id)?wp_send_json_success('Synced.'):wp_send_json_error('Failed. Check QBO connection.');
    }

    public static function ajax_assign_bank_txn() {
        check_ajax_referer('hearmed_nonce','nonce');
        $id=intval($_POST['txn_id']??0);$acc=sanitize_text_field($_POST['account_id']??'');
    ...
    private static function payment_badge($status) {
        $map=['Paid'=>'hm-badge--green','paid'=>'hm-badge--green','Partially Paid'=>'hm-badge--orange','Unpaid'=>'hm-badge--red','unpaid'=>'hm-badge--red','Refunded'=>'hm-badge--grey'];
